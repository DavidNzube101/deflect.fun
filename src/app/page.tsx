...
// Import Solana wallet styles - this is allowed in Next.js client components
...
const styles: Record<string, React.CSSProperties> = {
  container: {
    ...
    overflow: 'hidden',
    fontFamily: '"BBHBartle", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
    position: 'relative'
  },
  loadingScreen: {
    width: '100%',
...
const DeflectGame: React.FC = () => {
  ...
  // Initialize Styles and Timers
  useEffect(() => {
    // Inject global keyframes and fonts
    const styleSheet = document.createElement("style");
    styleSheet.type = "text/css";
    styleSheet.innerText = `
      @font-face {
        font-family: 'BBHBartle';
        src: url('${API_BASE}/assets/fonts/BBH_Bartle/BBHBartle-Regular.ttf') format('truetype');
      }
      * {
  ...
  const loadAllAssets = async () => {
    try {
      ...
      for (let i = 0; i < audioNames.length; i++) {
        ...
        audio.preload = 'auto';
        // Removed await canplaythrough for better robustness on mobile/slow nets, just proceed
      ...
      // Force entry even if assets fail
  ...
  const startGame = () => {
    ...
    // Reset lastUpdateRef to prevent large delta on start
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      setThreats(prev => {
        const baseSpeed = slowMo ? 0.005 : 0.015;
        const speedMultiplier = Math.pow(1.6, Math.floor(score / 500));
  ...
  const renderLoading = () => (
    <div style={styles.loadingScreen}>
      ...
      <p style={styles.loadingText}>{Math.floor(loadingProgress)}%</p>
    </div>
  );
            ...
            <div style={styles.detailsInfo}>
              ...
              {userData && (
                (() => {
                  ...
                  // Check locks in details view
                  let locked = false;
                  let lockMsg = '';
                  const totalGames = userData?.totalGames || 0;
                  
                  if (viewingItem.id === 'tate' || viewingItem.id === 'toly') {
                     if (totalGames < 60) { locked = true; lockMsg = 'Unlock at 60 Games'; }
                  } else if (viewingItem.id === 'infinite') {
                     if (totalGames < 50) { locked = true; lockMsg = 'Unlock at 50 Games'; }
                  } else if (viewingItem.id === 'god_revive') {
                     if (totalGames < 45) { locked = true; lockMsg = 'Unlock at 45 Games'; }
                  }

                  if (locked) {
                    return (
                        <button style={{ ...styles.detailsButton, backgroundColor: 'grey', cursor: 'not-allowed' }} disabled>
                          {lockMsg}
                        </button>
                    );
                  }
  ...
  const renderStore = () => (
    <div style={styles.storeScreen}>
      ...
      <div style={styles.glassCard}>
        <h2 style={{ ...styles.sectionTitle, fontSize: '18px' }}>CHARACTERS</h2>
        <div style={styles.storeCarousel}>
          {Object.values(allCharacters).map(char => {
            ...
            let locked = false;
            let lockMsg = '';
            if (char.id === 'tate' || char.id === 'toly') {
               if ((userData?.totalGames || 0) < 60) {
                 locked = true;
                 lockMsg = 'Unlock at 60 Games';
               }
            }
            ...
            return (
              <div key={char.id} style={styles.storeItem}>
                ...
                <div style={styles.storeActions}>
                  ...
                  {locked ? (
                    <button style={{ ...styles.storeButton, backgroundColor: 'grey', cursor: 'not-allowed' }} disabled>
                      {lockMsg}
                    </button>
                  ) : owned ? (
                    selected ? (
      ...
      <div style={{ ...styles.glassCard, marginTop: '20px' }}>
        <h2 style={{ ...styles.sectionTitle, fontSize: '18px' }}>POWERUPS</h2>
        <div style={styles.storeCarousel}>
          {Object.values(allPowerups).map(pow => {
            ...
            let lockMsg = '';
            if (pow.id === 'infinite') {
               if ((userData?.totalGames || 0) < 50) {
                 locked = true;
                 lockMsg = 'Unlock at 50 Games';
               }
            } else if (pow.id === 'god_revive') {
               if ((userData?.totalGames || 0) < 45) {
                 locked = true;
                 lockMsg = 'Unlock at 45 Games';
               }
            }
  ...
  const renderAirdropModal = () => (
    <div style={styles.overlay} onClick={() => setIsAirdropModalOpen(false)}>
      <div style={{...styles.glassCard, maxWidth: '600px'}} onClick={e => e.stopPropagation()}>
        ...
        <p style={{fontFamily: "sans-serif", color: 'white', textAlign: 'center'}}>
          Airdrops will be distributed to all active players at the end of the season. Only the top 15 players on the leaderboard will take part in this reward!
        </p>
        <p style={{fontFamily: "sans-serif", color: 'white', textAlign: 'center', marginTop: '10px'}}>
          Snapshots are taken on the <strong>25th of every month at 15:00 UTC</strong>.
        </p>
        <p style={{fontFamily: "sans-serif", color: 'white', textAlign: 'center', marginTop: '20px', fontWeight: 'bold'}}>
...
const DeflectApp: React.FC = () => {
  ...
  // Prevent hydration errors by ensuring wallet stuff runs only on client
